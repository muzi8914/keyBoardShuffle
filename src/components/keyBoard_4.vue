      ...
      <section class="floatKeyBoard" v-if="isShow">
         <div ref="ul" class="list flex flex_wrap flex_vcenter">
            <ul v-for="(i,k) in keyBoard" :key="k" class="ul">
               <li v-if="k==3" class="li flex flex_center flex_vcenter close" @click="tapNum(i,k,$event)">确定</li>
   ...
   props: {
	...
	methods: {
      ...
      shuffle(array){
         ...
         let arr1 = array.slice(0,3),arr2 = array.slice(3)
         arr1.push('')
         arr2.push('')
         this.keyBoard = arr1.concat(arr2)
      ...
      tapNum(i,k,e) {
         ...
         if (k==3) {
            let hidde = false
            this.$emit('changeKeyBoardStatus',hidde)
            return
         } else if (k==11) {
